...
class ConsoleRenderer(Renderer):
    def render(self, game_state: GameState) -> None:
        ...
        if game_state.winner:
            ...
            print(f"{game_state.winner} wins \N{party_popper}")
        else:
            print_solid(game_state.grid.cells)
            if game_state.tie:
                print("No one wins this time \N{neutral_face}")
    ...
    # should this be return f"\033[5m{text}\033]0m"

    def print_blinking(cells: Iterable[str], positions: Iterable[str]) -> None:
